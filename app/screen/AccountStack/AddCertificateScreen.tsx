...
const AddCertificateScreen: React.FC<Props> = ({
  ...
  const [experience, setExperience] = useState<Certificate>(
    isCreating
      ? {
  ...
  const [currentDateKey, setCurrentDateKey] = useState<keyof Certificate>();
  const [image, setImage] = useState<string>('');

  const changeValue = (key: keyof Certificate, val: string | number) => {
  ...
  const onPost = () => {
    if (
      experience.name !== '' &&
      experience.issuingOrganisation !== '' &&
      experience.issueDate !== ''
    ) {
      if (image.length > 0) {
        uploadAvatar(
          image,
          ...
          uri => {
            const res: UserInfo = {
              ...user,
              certificates: isCreating
                ? [{...experience, image: uri}, ...(user.certificates || [])]
    ...
    <Screen backgroundColor={'background2'} edges={['top']}>
      <View
        ...
        <Flex
          ...
          <Text
            ...
            numberOfLines={1}>
            {isCreating
              ? (t('add_certificate') as string)
              : (t('edit_certificate') as string)}
          </Text>
        </Flex>
      ...
      <KeyboardAwareScrollView
        ...
        <TextInput_
          ...
          autoComplete="name"
          color={'textPrimary'}
          placeholder={t('certification_placeholder') as string}
        ...
        <TextInput_
          ...
          autoComplete="name"
          color={'textPrimary'}
          placeholder={t('orgatisation_placeholder') as string}
          style={{width: '100%', color: theme.colors.textPrimary}}
          value={experience.issuingOrganisation}
          onBlur={(): void => {}}
          onChangeText={text => changeValue('issuingOrganisation', text)}
        />
        <Text
          mt={'spacing20'}
          mb={'spacing10'}
        ...
        <Flex
          ...
          <Text variant={'buttonLabelSmall'} color={'textPrimary'}>
            {t('get_certificate_badge')}
          </Text>
          <Text variant={'bodyMicro'} color={'textPrimary'}>
            {t('get_certificate_badge_desc')}
          </Text>
          <Text variant={'bodyMicro'} color={'textPrimary'}>
