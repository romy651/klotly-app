...
const BiSectionList = () => {
    ...
    const headers = React.useMemo(() => DATA.map(item => ({key: item.key, title: item.title})), [])
    const [index, setIndex] = React.useState(0)

    const onCheckViewableItems = ({viewableItems, changed}: {viewableItems: any; changed: any}) => {
        const half = viewableItems.slice(Math.floor(viewableItems.length / 2), -1)
        //const lastIndex = findLastIndex(viewableItems, i => i.index === 0 || i.index === null)
        //@ts-ignore
        const lastIndex = viewableItems.findLastIndex(i => i.index === 0 || i.index === null)
